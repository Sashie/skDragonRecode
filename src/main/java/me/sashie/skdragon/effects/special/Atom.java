...
public class Atom extends SpecialRadiusDensityEffect implements IExtra, IStep {
	...
	public void update(DynamicLocation location) {
		...
		float densityAngle = MathUtils.PI / this.getDensityProperty().getDensity(2);
		float angularVelocity = MathUtils.PI / this.getExtraProperty().getValue(1);

		for (int i = 0; i < this.getDensityProperty().getDensity(1); i++) {
			float angle = step * angularVelocity;
			for (int j = 0; j < this.getDensityProperty().getDensity(2); j++) {
				float xRotation = densityAngle * j;
				vector.setX(MathUtils.sin(angle) * ( 0.6 + this.getRadiusProperty().getRadius(2) ));
				vector.setY(MathUtils.cos(angle) * ( 0.6 + this.getRadiusProperty().getRadius(2) ));
				vector.setZ(0);
	...
	public ParticleBuilder<?>[] defaultParticles() {
		return new ParticleBuilder<?>[] { new ColoredParticle(ParticleUtils.REDSTONE), new ColoredParticle(ParticleUtils.REDSTONE) };
	}
