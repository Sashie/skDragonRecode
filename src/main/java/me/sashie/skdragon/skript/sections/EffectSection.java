...
import me.sashie.skdragon.project.util.ReflectionUtils;
...
public abstract class EffectSection extends Condition {
	...
	public static void stopLog(RetainingLogHandler logger) {
		...
		HandlerList handler = SkDragonRecode.getSkriptAdapter().getHandlers();//ReflectionUtils.getField(SkriptLogger.class, null, "handlers");
		if (handler == null)
			return;
		...
		while (it.hasNext()) {
			...
			if (l instanceof ParseLogHandler)
				toStop.add(l);
			else //We can only stop the last handler, this prevents it in case the last is not what we want.
				break;
		}
